...
class VoucherVoucher(models.Model):
	...
	def check_positive_validity(self):
		if self.issue_date < date.today():
			raise UserError('Applicable from date should not less than today date')
		if self.expiry_date < self.issue_date:
			raise UserError('Expiry date should not less then issue date')
	...
	def get_default_values(self, field=False):
		...
		vals['wk_coupon_name'] =  irDefault.get('res.config.settings', 'wk_coupon_name') or 'Default Coupon Name'
		return vals
	...
	@api.constrains('redeemption_limit')
	def check_redeemption_limit(self):
		if self.redeemption_limit == 0 or self.redeemption_limit < -1:
			raise ValidationError('The redeemption limit should not be zero or less than -1')
	...
	def send_mail_to_customers(self):
		if self.customer_type == 'special_customer':
			...
			template_obj = self.env.ref('wk_coupons.mail_template_voucher_voucher')
			values = {}
			values['email_to'] = self.customer_id.email
			res = template_obj.send_mail(self.id,force_send=True ,email_values=values)
			if res:
	...
	def create(self, vals):
		...
		if self.applied_on == "specific":
			if not self.product_ids:
				raise ValidationError('Please add the specific products')
	...
	def write(self, vals):
		for voucher in self:
			...
			if  vals.get('voucher_value') and vals.get('voucher_value') > self.get_default_values('wk_coupon_max_amount'):
				raise ValidationError(('You can`t create a voucher greater than maximum amount (%s) !!!')%self.get_default_values('wk_coupon_max_amount'))
	...
	@api.constrains('applied_on','product_ids')
	def check_specific_products(self):
		if self.applied_on == "specific":
				if not self.product_ids:
					raise ValidationError('Please add the specific products')
